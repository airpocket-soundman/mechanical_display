...
#                        [1830, 1940, 1840, 1970,  1920, 1940, 1920, 1770,  1870, 1850, 1860, 1770,  1900, 1870, 1910, 1990],
                        [1870, 2030, 1880, 2035,  1930, 1940, 1970, 1830,  1910, 1880, 1900, 1830,  1940, 1920, 1965, 2040],
...
# 全てのサーボをリリースするメソッド
    def release(self, coordinate = None):
        #全てのサーボをリリース
        if coordinate == None:
...
#                print("image size unmatched")
    ...
    def fontOverlay(self, bg_image, font_image ,offset, font_color = None, transparent = True):
        ...
        for y in range(font_size[1]):
            if y + offset[1] < bg_size[1]:
                for x in range(font_size[0]):
                    if x + offset[0] < bg_size[0]:
                        if transparent == False:
                            image[x + offset[0]][y + offset[1]] = font_image[x][y] * font_color
                        else:
                            if font_image[x][y] == 1:
                                image[x + offset[0]][y + offset[1]] = font_image[x][y] * font_color
        return image
    ...
    def textOverlay(self, text_image ,offset = [0,0], text_color = None, bg_color = None, transparent = True):
        ...
        display_size = self.pixel_layout
